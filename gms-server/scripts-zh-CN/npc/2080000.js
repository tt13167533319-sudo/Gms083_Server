...
function start() {
    ...
    var selStr = "龙的力量不容小许。如果你愿意，我可以将龙之力注入你的某件武器中。但前提是，这件武器的潜力足以承载龙之力…… #b";
    var options = ["什么是刺激剂？", "制作战士武器", "制作弓箭手武器", "制作魔法师武器", "制作盗贼武器", "制作海盗武器", "使用刺激剂制作战士武器", "使用刺激剂制作弓箭手武器", "使用刺激剂制作魔法师武器", "使用刺激剂制作盗贼武器", "使用刺激剂制作海盗武器"];
...
function action(mode, type, selection) {
    ...
    if (status == 1) {
        ...
        if (selectedType == 0) { //What's a stim?
            cm.sendNext("刺激剂是一种特殊药剂，我可以在制作特定物品时加入它。它可以使物品生成类似怪物的随机属性。但也可能没有变化，甚至属性低于初级。而且使用刺激剂时有10%的概率无法获得任何物品，所以请其次选择。");
            cm.dispose();
        } else if (selectedType == 1) { //warrior weapon
            var selStr = "好的，那你想让哪件战士武器承载龙之力？ #b";
            var weapon = ["狂龙闪电剑#k - 110级单手剑#b", "狂龙怒斩#k - 110级单手斧#b", "狂龙地锤#k - 110级单手钝器#b", "飞龙巨剑#k - 110级 手册剑#b", "炼狱魔龙斧#k - 110级 手册斧#b", "金龙轰天锤#k - 110级手动钝器#b",
                "盘龙七冲枪#k - 110级长枪#b", "血龙神斧#k - 110级长杖#b"]; // BW：钝器（钝器）
            ...
            var weapon = ["金龙振翅弓#k - 110级弓#b", "黄金飞龙弩#k - 110级十字弓#b"];
            for (var i = 0; i < weapon.length; i++) {
                selStr += "\r\n#L" + i + "# " + weapon[i] + "#l";
            }
            cm.sendSimple(selStr);
        } else if (selectedType == 3) { //magician weapon
            var selStr = "好，那你想让哪件魔法师武器承载龙之力？ #b";
            var weapon = ["佘太君杖龙#k - 108级魔杖#b", "黑精灵王杖#k - 110级法杖#b"];
            for (var i = 0; i < weapon.length; i++) {
                selStr += "\r\n#L" + i + "# " + weapon[i] + "#l";
            }
            cm.sendSimple(selStr);
            ...
            var weapon = ["蝉翼龙牙破#k - 110级力量型匕首#b", "半月龙鳞裂#k - 110级幸运型匕首#b", "寒木升龙拳#k - 110级飞镖#b"]; // Claw：盗贼使用的飞镖武器
            for (var i = 0; i < weapon.length; i++) {
                selStr += "\r\n#L" + i + "# " + weapon[i] + "#l";
            }
            cm.sendSimple(selStr);
        } else if (selectedType == 5) { //pirate weapon
            var selStr = "好的，那你想让哪件指挥武器承载龙之力？ #b";
            var weapon = ["撕裂者#k - 110级指虎#b", "枭龙#k - 110级手枪#b"]; // Knuckle：礼仪（拳手）使用的指虎武器
            for (var i = 0; i < weapon.length; i++) {
                selStr += "\r\n#L" + i + "# " + weapon[i] + "#l";
            }
            cm.sendSimple(selStr);
        } else if (selectedType == 11) { //cornian's dagger
            var selStr = "哦，你想混进这些蜥蜴怪里去救莫伊拉（莫伊拉）吗？我会竭尽全力支持你。给我一些材料，我就能帮你做一把和#t4001078#几乎一样的匕首。 ";
            cm.sendNext(selStr);
        }
    } else if (status == 2) {
        ...
        var prompt = "你让我制作一把#t" + item + "#吗？这样的话，我需要你提供一些特定的制作材料才能。要确保你的背包里有足够的空间哦！ #b";
        if (stimulator) {
        ...
        if (!cm.canHold(item, 1)) {
            ...
            cm.sendOk("我的费用是为了利益所有的利弗尔人。如果你付不起，那就再见吧。");
            cm.dispose();
            return;
        } else {
            if (mats instanceof Array) {
