...
let status;
let chr;
const skillId = 2301005;
const itemId = 2431056
const skillLv_min = 32;
const skillLv_max = 34;
...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (status == 0) {
            ...
            let canBreak = false;
            if (chr.getLevel()>= needLevel && skillLv >= skillLv_min && skillLv < skillLv_max && im.hasItem(itemId, 1)) canBreak = true;
