...
public class MapItem extends AbstractMapObject {
    ...
    // 新增独立掉落属性
    private boolean isSoloDrop;  // 是否为独立掉落
    private int soloOwnerId;     // 独立掉落的所有者ID（仅当isSoloDrop为true时有效）
    ...
    // 新增setter和getter
    public void setSoloDrop(boolean soloDrop) {
        isSoloDrop = soloDrop;
    }

    public boolean isSoloDrop() {
        return isSoloDrop;
    }

    public void setSoloOwnerId(int ownerId) {
        this.soloOwnerId = ownerId;
    }

    public int getSoloOwnerId() {
        return soloOwnerId;
    }
    ...
    public final boolean canBePickedBy(Character chr) {
        // 独立掉落物：仅允许所有者拾取，忽略原有派对/过期逻辑
        if (isSoloDrop) {
            return chr.getId() == soloOwnerId;
        }
...
//        if (isSoloDrop && soloOwnerId == client.getPlayer().getId()) {
//            this.lockItem();
//            try {
//                client.sendPacket(PacketCreator.dropItemFromMapObject(chr, this, dropper.getPosition(), getPosition(), (byte) 1));
//            } finally {
//                this.unlockItem();
//            }
//        }
