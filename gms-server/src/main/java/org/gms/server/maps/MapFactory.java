            ...
            if (monster.isBoss()) {
                ...
                if (MobId.isAreaBoss(mobId)){
                    switch (mobId){
                        ...
                        case MobId.玄冰独角兽:
                        case MobId.雷卡:
                            int randomTimeCommon = GameConfig.getServerShort("area_boss_respawn_random_time");
                            mobTimeTemp = GameConfig.getServerShort("area_boss_respawn_time") * 60 + (ThreadLocalRandom.current().nextInt(randomTimeCommon * 60 * 2 + 1) - (randomTimeCommon * 60));
                            break;
                        case MobId.皮亚奴斯左:
                        case MobId.皮亚奴斯右:
                            int randomTimeSpecial = GameConfig.getServerShort("special_area_boss_respawn_random_time");
                            mobTimeTemp = GameConfig.getServerShort("special_area_boss_respawn_time") * 3600 + (ThreadLocalRandom.current().nextInt(randomTimeSpecial * 60 * 2 + 1) - (randomTimeSpecial * 60));;
                            break;
                        default:
                            mobTimeTemp = mobTime;
                                break;
                ...
                mobTime = NumberTool.floatToInt(mobTimeTemp * mobTimeRate);
            }
            // 如果是事件地图，刷新倍率保持不变
