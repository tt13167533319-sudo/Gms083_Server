...
//                if (ItemInformationProvider.getInstance().getName(itemId) == null ) System.out.print("getName [" + itemId + "] is null\r\n");
//                if (ItemInformationProvider.getInstance().getEquipStats(itemId) == null) System.out.print("getEquipStats [" + itemId + "] is null\r\n");
//                Equip equip = (Equip) ItemInformationProvider.getInstance().getEquipById(itemId);
//                if (equip.getDex() + equip.getLuk() + equip.getStr() + equip.getInt() + equip.getMatk() + equip.getWatk() + equip.getHp() + equip.getMp() > 0) System.out.print("equip [" + itemId + "] is special\r\n");
...
//            ModifiedCashItemDO returnDo = cashItemDO.clone();
            ...
            // 1. 先从WZ文件加载的items中获取基础数据
            ModifiedCashItemDO cashItemDO = items.get(sn);
            // 2. 若WZ中无数据，尝试从数据库的modifiedCashItems中获取
            if (cashItemDO == null) {
                cashItemDO = modifiedCashItems.get(sn);
                // 若数据库中也无数据，返回null
                if (cashItemDO == null) {
                    return null;
                }
                // 数据库中有数据但WZ中无，直接克隆数据库数据返回（无需合并，因无基础数据）
                return cashItemDO.clone();
            }

            // 3. 若WZ中有数据，合并数据库中的修改（原有逻辑保留）
            ModifiedCashItemDO dbItemDO = modifiedCashItems.get(sn);
            ModifiedCashItemDO returnDo = cashItemDO.clone();
            if (dbItemDO != null) {
