...
public class ItemInformationProvider {
    ...
    private Data getStringData(int itemId) {
        ...
        } else if ((itemId >= 1010000 && itemId < 1040000) || (itemId >= 1120000 && itemId < 1150000)) {
            //101：脸 102：眼 103：耳环 112：项链 113：腰带 114：勋章
    ...
    private static short getExtraSlotMaxFromPlayer(Client c, int itemId) {
        ...
        int skillLv = 0;
        if (ItemConstants.isThrowingStar(itemId)) {//飞镖
            if (c.getPlayer().getJob().isA(Job.NIGHTWALKER1)) {
                skillLv = c.getPlayer().getSkillLevel(SkillFactory.getSkill(NightWalker.CLAW_MASTERY));
            } else {
                skillLv = c.getPlayer().getSkillLevel(SkillFactory.getSkill(Assassin.CLAW_MASTERY));
            }
        } else if (ItemConstants.isBullet(itemId)) {//子弹
            skillLv = c.getPlayer().getSkillLevel(SkillFactory.getSkill(Gunslinger.GUN_MASTERY));
        }

        if (skillLv <= 20) ret += skillLv * 10;
        else if (skillLv == 21) ret += 600;
        else if (skillLv == 22) ret += 1200;

        return ret;
    }

    public boolean isSpecialSlotItem(int itemId) {
        return switch (itemId) {
            case 2060000, 2060001, 2060002, 2060003, 2060004, 2060008, 2060009, 2060010, 2060011,//弓矢
                 2061000, 2061001, 2061002, 2061003, 2061004, 2061006, 2061007, 2061008, 2061009,//弩矢
                 2070000, 2070001, 2070002, 2070003, 2070004, 2070005, 2070006, 2070007, 2070008, 2070009, 2070010, 2070011, 2070012, 2070013, 2070015, 2070016, 2070018, 2070019, 2070023, 2070024, 2070026//飞镖
                    -> true;
            default -> false;
        };
    }
    ...
    public short getSlotMax(Client c, int itemId) {
        ...
        if (item != null) {
            ...
            if (smEntry == null) {
                ...
                ret = inventoryType.canChangeSlotMax() && !isSpecialSlotItem(itemId) && itemSlotMax > 0 ? itemSlotMax : (short) DataTool.getInt(smEntry);
            }
        }
    ...
    */
    ...
    public Map<String, Integer> getSkillStats(int itemId, double playerJob) {
        ...
        for (int i = 0; i < skill.getChildren().size(); i++) {
            ...
            if (curskill / 10000 == playerJob || curskill == 1013) {
