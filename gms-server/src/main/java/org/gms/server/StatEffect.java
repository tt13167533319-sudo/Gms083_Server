...
//                case Beginner.SPACESHIP:
                ...
                case Marksman.FROST_PREY:
                    ...
                    break;
                case Beginner.SAMSARA_STELE:
                    statups.add(new Pair<>(BuffStat.SAMSARA_STELE, 1));
                    break;
    ...
    private boolean applyTo(Character applyfrom, Character applyto, boolean primary, Point pos, boolean useMaxRange, int affectedPlayers) {
        ...
        if (GameConfig.getServerBoolean("use_debug_buff_skill") && applyfrom.isGM()){
            applyfrom.dropMessage(5,"[调试信息] 应用技能，ID [" + sourceid + "]");
        }
        ...
        if (primary) {
            ...
            if (isResurrection()) { //复活术
                hpchange = applyto.getCurrentMaxHp();
                applyto.broadcastStance(applyto.isFacingLeft() ? 5 : 4);
            }
        }

        if (isDispel() && makeChanceResult()) {//净化
            applyto.dispelDebuffs();
        } else if (isCureAllAbnormalStatus()) {//勇士的意志 和 万能药水
            applyto.purgeDebuffs();
        } else if (isComboReset()) {//战神之盾 吸血
            ...
            try {
                ...
                for (int i = 1; i <= use.getSlotLimit(); i++) { // impose order...
                    ...
                    if (item != null) {
                        ...
                        if (ItemConstants.isThrowingStar(itemid) && item.getQuantity() >= projectileConsume) {
                            if (itemid == ItemId.BALANCED_FURY && item.getQuantity() <= projectileConsume) {//匹配平衡之怒
                                projectileConsume = (short) -ItemInformationProvider.getInstance().getSlotMax(applyto.getClient(), itemid);  //设定补充到限制的最高数值
                            }
                            projectile = item;
                            break;
                        }
        ...
        if (overTime || isCygnusFA() || summonMovementType != null) { //终极攻击？
        ...
        if (summonMovementType != null && pos != null) {//召唤兽？
        ...
        if (isMagicDoor() && !FieldLimit.DOOR.check(applyto.getMap().getFieldLimit())) { // 传送门
            int y = applyto.getFh();
            if (y == 0) {
        ...
        //新增敛财术额外触发效果
        if (sourceid == ChiefBandit.PICKPOCKET) {
            int skillId = GameConfig.getServerInt("pick_pocket_special_effect");
            if (skillId > 0) {
                int skilllv = applyfrom.getSkillLevel(ChiefBandit.PICKPOCKET);
                if (skilllv > 20) {
//                    SkillFactory.getSkill(skillId).getEffect(20).applyToDuration(applyfrom,duration/1000);
                    Skill skill = SkillFactory.getSkill(skillId);
                    if (skilllv == 21) skill.getEffect(skill.getBreakMaxLevel() - 1).applyToDuration(applyfrom, duration / 1000);
                    else if (skilllv == 22) skill.getEffect(skill.getBreakMaxLevel()).applyToDuration(applyfrom, duration / 1000);
                }
            }
        }

        if (sourceid == Cleric.BLESS) {//祝福
            applyto.setBlessLevel(applyfrom.getSkillLevel(Cleric.BLESS));
        }
        if (sourceid == Bishop.INFINITY || sourceid == ILArchMage.INFINITY || sourceid == FPArchMage.INFINITY){//终极无限
            int skilllv = applyfrom.getSkillLevel(sourceid);
            if (skilllv > 30) {
                SkillFactory.getSkill(21120007).getEffect(skilllv).applyToDuration(applyfrom,duration/1000);
            }
        }
    ...
    public boolean applyToDuration(Character chr, int duration) {
        return applyToDuration(chr, chr, true, null, false, 1, duration);
    }

    private boolean applyToDuration(Character applyfrom, Character applyto, boolean primary, Point pos, boolean useMaxRange, int affectedPlayers, int duration) {
        this.duration = duration * 1000;
        return applyTo(applyfrom, applyto, primary, pos, useMaxRange, affectedPlayers);
    }

    private int applyBuff(Character applyfrom, boolean useMaxRange) {
        int affectedc = 1;

        if (isPartyBuff(applyfrom.getJob().getId()) && (applyfrom.getParty() != null || isGmBuff())) {
    ...
    private void applyMonsterBuff(Character applyfrom) {
        ...
        for (MapObject mo : affected) {
            ...
            } else if (isSeal() && monster.isBoss()) {//封印术
                // do nothing
            } else {
                if (makeChanceResult()) {
                    monster.applyStatus(applyfrom, new MonsterStatusEffect(getMonsterStati(), skill_, null, false), isPoison(), getDuration());
                    if (isCrash()) { //防御崩坏
                        int skillLv = applyfrom.getSkillLevel(skill_);
                        if (skillLv > 20) monster.debuffMob(skill_.getId(), skillLv);
                        else monster.debuffMob(skill_.getId());
                    }
                }
            }
            i++;
...
//            } else if (sourceid == Beginner.SPACESHIP || sourceid == Noblesse.SPACESHIP) {
            } else if ( sourceid == Noblesse.SPACESHIP) {
    ...
    private boolean isPartyBuff(int jobid) {
        if (lt == null || rb == null) {
            return false;
        }
        // wk charges have lt and rb set but are neither player nor monster buffs
        if (sourceid == Fighter.RAGE && !GameConfig.getServerBoolean("fighter_rage_apply_party")) return false;//愤怒之火
        if ((sourceid == Bowmaster.SHARP_EYES || sourceid == Marksman.SHARP_EYES) && !GameConfig.getServerBoolean("sharp_eyes_apply_party")) return false;//火眼晶晶
        if ((sourceid == Hermit.MESO_UP || sourceid == ChiefBandit.PICKPOCKET) && !GameConfig.getServerBoolean("meso_up_apply_party")) return false;//敛财术、聚财术
        if (sourceid == Aran.COMBO_BARRIER && jobid >=120 && jobid <= 122) return false; //压制术 吸收之盾
...
//        return skill && (sourceid % 10000000 == 1004 || sourceid == Corsair.BATTLE_SHIP || sourceid == Beginner.SPACESHIP || sourceid == Noblesse.SPACESHIP
        return skill && (sourceid % 10000000 == 1004 || sourceid == Corsair.BATTLE_SHIP  || sourceid == Noblesse.SPACESHIP
