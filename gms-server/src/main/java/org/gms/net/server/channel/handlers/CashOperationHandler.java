            ...
            try {
                ...
                if (action == 0x03 || action == 0x1E) { // buy
                    ...
                    if (!canBuy(chr, cItem, cs.getCash(useNX))) {
                        if (cItem != null) {
                            log.error("尝试购买禁止被出售的道具[{}], 角色[{}({})], SN [{}]", cItem.getItemId(), chr.getName(), chr.getId(), snCS); // preventing NPE here thanks to MedicOP
                        } else {
                            log.error("尝试购买禁止被出售的道具[null], 角色[{}({})], SN [{}]", chr.getName(), chr.getId(), snCS); // preventing NPE here thanks to MedicOP
                        }
//                        c.sendPacket(PacketCreator.showCashShopMessage((byte) 0xE6));
                    ...
                    String noteMessage = chr.getName() + " 送给你一件礼物! 请前往商城查看.";
    ...
    private static boolean canBuy(Character chr, ModifiedCashItemDO item, int cash) {
        if (item != null && item.isSelling() && item.getPrice() <= cash) {
            Equip equip = (Equip) ItemInformationProvider.getInstance().getEquipById(item.getItemId());
            if (equip.getDex() + equip.getLuk() + equip.getStr() + equip.getInt() + equip.getMatk() + equip.getWatk() + equip.getHp() + equip.getMp() > 0) return false;
            log.info("玩家 {} 购买了现金道具 {} (ID:[{}] SN:[{}]) 花费 {}", chr, ItemInformationProvider.getInstance().getName(item.getItemId()), item.getItemId(), item.getSn(), item.getPrice());
            return true;
        } else {
            return false;
        }
