...
public final class ChangeMapHandler extends AbstractPacketHandler {
    ...
    public void handlePacket(InPacket p, Client c) {
        ...
        try {
            ...
            if (GameConfig.getServerBoolean("open_mouse_fly_check") && portal != null) {
                Point2D portalPos = portal.getPosition();
                double xDifference = Math.abs(portalPos.getX() - chr.getPosition().getX());
                double yDifference = Math.abs(portalPos.getY() - chr.getPosition().getY());
                if (portalPos.distanceSq(chr.getPosition()) > GameConfig.getServerInt("mouse_fly_check_total") || xDifference > GameConfig.getServerInt("mouse_fly_check_x") || yDifference > GameConfig.getServerInt("mouse_fly_check_y")) {
                    log.info("portalPos.getX() = " + portalPos.getX());
                    log.info("portalPos.getY() = " + portalPos.getY());
                    log.info("chr.getPosition().getX() = " + chr.getPosition().getX());
                    log.info("chr.getPosition().getY() = " + chr.getPosition().getY());
                    log.info("xDifference = " + xDifference);
                    log.info("yDifference = " + yDifference);
                    log.info("portalPos.distanceSq(chr.getPosition()) = " + portalPos.distanceSq(chr.getPosition()));
                    AutobanFactory.PORTAL_DISTANCE.addPoint(chr.getAutoBanManager(), "鼠标飞天");
                    c.sendPacket(PacketCreator.enableActions());
                    return;
                }
            }
