...
public class Server {
    ...
    // 新增：保存重生任务的调度引用
    private ScheduledFuture<?> respawnTaskFuture;
    ...
    private int initWorld() {
        ...
        short world_level = GameConfig.getWorldShort(i, "world_level");
        boolean only_allow_gm_login = GameConfig.getWorldBoolean(i, "only_allow_gm_login");
        ...
        World world = new World(i, flag, event_message, expRate, dropRate, bossDropRate, mesoRate, questRate,
                travelRate, fishingRate, world_level, only_allow_gm_login);
        ...
        if (world_level > 0) log.info(I18nUtil.getLogMessage("Server.initWorld.info3"), i, world_level);

        if (canDeploy) {
            world.setServerMessage(GameConfig.getWorldString(i, "server_message").equals("null") ? "" : GameConfig.getWorldString(i, "server_message"));
...
//        final int worldCount = Math.min(GameConstants.WORLD_NAMES.length, GameConfig.getConfig().getJSONObject("world").size());
        final int worldCount = Math.min(GameConstants.WORLD_NAMES.length, GameConfig.getServerByte("world_count"));
        ...
        online = true;
        SessionCoordinator.getInstance().loadMultiClientExcept();
    ...
    private void initializeTimelyTasks() {
        ...
        //修改：保存重生任务的调度结果
        long respawnInterval = GameConfig.getServerLong("respawn_interval");
        respawnTaskFuture = tMan.register(
                new RespawnTask(),
                respawnInterval,
                respawnInterval
        );
    ...
    // 新增：提供更新重生间隔的方法
    public void updateRespawnInterval(long newInterval) {
        if (respawnTaskFuture == null) {
            return;
        }
        TimerManager tMan = TimerManager.getInstance();
        // 使用TimerManager的update方法动态更新间隔
        respawnTaskFuture = tMan.update(
                respawnTaskFuture,
                new RespawnTask(),  // 新的任务实例（逻辑与原任务一致）
                newInterval
        );
    }
