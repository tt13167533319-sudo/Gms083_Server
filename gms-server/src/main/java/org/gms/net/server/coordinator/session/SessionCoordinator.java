...
public class SessionCoordinator {
    ...
    private List<String> multiClientExcept = new ArrayList<>();
    private SessionCoordinator() {
    }

    public void loadMultiClientExcept(){
        multiClientExcept.clear();
        String listStr = GameConfig.getServerString("multi_client_except");
        if (listStr != null && !listStr.trim ().isEmpty ()) {
            String [] items = listStr.split (",");
            for (String item : items) {
                String trimmedItem = item.trim ();
                if (!trimmedItem.isEmpty ()) {
                    multiClientExcept.add (trimmedItem);
                }
            }
        }
    }

    private boolean isExceptAcc (String accName){
        Iterator<String> iterator = multiClientExcept.iterator();
        while (iterator.hasNext()) {
            String element = iterator.next();
            if (element.equals(accName)) {
                return true;
            }
        }
        return false;
    ...
    // 新增代理方法：供外部包调用，间接执行removeEntryAndGetItsHwid
    public Hwid removeHostHwidEntryAndGetHwid(String remoteHost) {
    ...
    public AntiMulticlientResult attemptLoginSession(Client client, Hwid hwid, int accountId, boolean routineCheck) {
        ...
        try {
            Hwid hwidtemp = SessionCoordinator.getInstance().getGameSessionHwid(client, (byte) 2);
            String accname = client.getAccountName();
            if (hwidtemp != null && !isExceptAcc(accname)) {
            ...
            if (routineCheck && !attemptAccountAccess(accountId, hwid, routineCheck)) {
                return AntiMulticlientResult.REMOTE_REACHED_LIMIT;
            }
            if (!attemptAccountAccess(accountId, hwid, routineCheck)) {
                return AntiMulticlientResult.REMOTE_REACHED_LIMIT;
            }
    ...
    private static Client fetchInTransitionSessionClient(Client client) {
        Hwid hwid = SessionCoordinator.getInstance().getGameSessionHwid(client, (byte) 1);
...
//        return hostHwidCache.removeEntryAndGetItsHwid(remoteHost);
    ...
    public Hwid getGameSessionHwid(Client client,Byte type) {
        String remoteHost = null;
        switch (type){
            case 1:
                remoteHost = getSessionRemoteHost(client);

                break;
            case 2:
                remoteHost = client.getRemoteAddress();
                break;
        }
        return hostHwidCache.getEntryHwid(remoteHost);
    }
