...
//                case 14: {
    ...
    protected boolean updatePosition(InPacket p, AnimatedMapObject target, int yOffset) throws EmptyMovementException {

        byte numCommands = p.readByte();
        boolean isCheck = true;
        List<Byte> checkList = new ArrayList<>();
        if (numCommands < 1) {
            throw new EmptyMovementException(p);
        }
        ...
        if (checkList.contains((byte)4)) isCheck = false;
        return isCheck;
    }
}
