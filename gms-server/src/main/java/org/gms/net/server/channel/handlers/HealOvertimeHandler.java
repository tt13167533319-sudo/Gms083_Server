...
//        int timestamp = Server.getInstance().getCurrentTimestamp();
        ...
        if (healHP != 0) {
            ...
            if ((abm.getLastSpam(8) + 1500) > timestamp) {
                AutobanFactory.FAST_HP_HEALING.alert(chr, "Fast hp healing");
            }
            ...
            abm.spam(8, timestamp);
        }
        short healMP = p.readShort();
        if (healMP != 0 && healMP < 1000) {
            long lastSapm = abm.getLastSpam(9);
            abm.spam(9, timestamp);
            abm.setTimestamp(9, timestamp, 28);
            if ((lastSapm + 1500) > timestamp) {
                System.out.println("lastSapm = " + lastSapm + "\r\n");
                System.out.println("abm.getLastSpam(9) = " + abm.getLastSpam(9) + "\r\n");
                System.out.println("timestamp = " + timestamp + "\r\n");
                AutobanFactory.FAST_MP_HEALING.alert(chr, "Fast mp healing");
