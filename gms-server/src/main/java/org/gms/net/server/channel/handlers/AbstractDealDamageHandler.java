        ...
        public StatEffect getAttackEffect(Character chr, Skill theSkill) {
            ...
            if (display > 80) { //Hmm
                if (!mySkill.getAction()) {
                    AutobanFactory.FAST_ATTACK.autoban(chr, "玩家["+ chr.getName() +"]攻击无延迟，实际延迟：" + display + "ms，技能ID：" + mySkill.getId());//?
                    return null;
                }
            }
        ...
        try {
            ...
            for (Integer oned : attack.allDamage.keySet()) {
                ...
                if (monster != null) {
                    ...
                    } else if (attack.skill == ILArchMage.CHAIN_LIGHTNING){
                        distanceToDetect += 200000;
                    }
                    if (distance > distanceToDetect && !player.isFullScreenAttacking()) {
                        AutobanFactory.DISTANCE_HACK.addPoint(player.getAutoBanManager(), "距离到怪物: " + distance + " 技能ID: " + attack.skill + " 怪物ID: " + monster.getId());
                    ...
                    if (player.getBuffedValue(BuffStat.PICKPOCKET) != null && (attack.skill == 0 || attack.skill == Rogue.DOUBLE_STAB || attack.skill == Bandit.SAVAGE_BLOW || attack.skill == ChiefBandit.ASSAULTER || attack.skill == ChiefBandit.BAND_OF_THIEVES || attack.skill == Shadower.ASSASSINATE || attack.skill == Shadower.TAUNT || attack.skill == Shadower.BOOMERANG_STEP)) {
                        if (player.getMapId() != 993184000) { //伤害测试地图不掉落金币 防止刷钱
                            Skill pickpocket = SkillFactory.getSkill(ChiefBandit.PICKPOCKET);//敛财术
                                ...
                                for (Integer eachd : onedList) {
                                    ...
                                    if (pickpocket.getEffect(picklv).makeChanceResult()) {
                                        ...
                                        TimerManager.getInstance().schedule(() -> map.spawnMesoDrop(
                                                        ...
                                                        new Point((int) (monster.getPosition().getX() + Randomizer.nextInt(100) - 50), (int) (monster.getPosition().getY())),
                                                        monster,
                                                        player,
                                                        true,
                                                        (byte) 2),
                    ...
                    } else if (attack.skill == Outlaw.HOMING_BEACON || attack.skill == Corsair.BULLSEYE) { //导航、导航辅助
                    ...
                    } else if (attack.skill == Outlaw.FLAME_THROWER) { //烈焰喷射
                        if (!monster.isBoss()) {
...
//                                        monster.applyStatus(player, monsterStatusEffect, false, duration);
                        ...
                        if (player.getBuffedValue(BuffStat.COMBO_DRAIN) != null) {
                            ...
                            int heal = ((totDamage * skill.getEffect(player.getSkillLevel(skill)).getX()) / 100);
                            player.addHP(heal);
                            player.sendPacket(PacketCreator.showOwnRecovery((byte) heal));
                            player.getMap().broadcastMessage(player, PacketCreator.showRecovery(player.getId(), (byte) heal), false);
                        }
                    } else if (player.hasActiveBuff(Cleric.BLESS)) { // 祝福吸血光环
                        int blessLevel = player.getBlessLevel();
                        if (blessLevel == 21 || blessLevel == 22) {
                            double healRate = blessLevel == 21 ? 0.01 : 0.03;
                            int maxHeal = blessLevel == 21 ? 100 : 250;

                            int heal = (int) Math.min(maxHeal, Math.ceil(Math.abs(totDamage * healRate)));
                            player.addHP(heal);
                            player.sendPacket(PacketCreator.showOwnRecovery((byte) heal));
                            player.getMap().broadcastMessage(player, PacketCreator.showRecovery(player.getId(), (byte) heal), false);
                        }
...
//            calcDmgMax = (long) (Math.ceil((chr.getTotalMagic() * Math.ceil(chr.getTotalMagic() / 1000.0) + chr.getTotalMagic()) / 30.0) + Math.ceil(chr.getTotalInt() / 200.0));
            calcDmgMax = (long) Math.ceil(((chr.getTotalMagic() * (10.0 / 3.0)
                    + chr.getTotalMagic() * chr.getTotalMagic() * 0.003365
                    + chr.getTotalInt() / 2.0)
                    / 100.0));
        } else if (ret.skill == Rogue.LUCKY_SEVEN || ret.skill == NightWalker.LUCKY_SEVEN || ret.skill == NightLord.TRIPLE_THROW || (chr.getJob().getId() >= 410 && chr.getJob().getId() <= 412 && ret.skill == 0)) {//标飞
            calcDmgMax = (long) ((chr.getTotalLuk() * 5.0) * (chr.getTotalWatk() / 100.0));
        } else if (ret.skill == DragonKnight.DRAGON_ROAR) {//龙咆哮
            calcDmgMax = (long) ((chr.getTotalStr() * 4.0 + chr.getTotalDex()) * (chr.getTotalWatk() / 100.0));
        } else if (ret.skill == NightLord.VENOMOUS_STAR || ret.skill == Shadower.VENOMOUS_STAB || ret.skill == NightWalker.VENOM) {//武器用毒液
//            calcDmgMax = (long) (Math.ceil((18.5 * (chr.getTotalStr() + chr.getTotalLuk()) + chr.getTotalDex() * 2) / 100.0) * chr.calculateMaxBaseDamage(chr.getTotalWatk()));
            ...
            poisonLevel = chr.getSkillLevel(SkillFactory.getSkill(ret.skill));
            matk = SkillFactory.getSkill(ret.skill).getEffect(poisonLevel).getMatk();
            int luk = chr.getLuk();
            calcDmgMax = (int) Math.ceil(Math.min(Short.MAX_VALUE, 0.2 * luk * matk * 3));
        } else {
            calcDmgMax = chr.calculateMaxBaseDamage(chr.getTotalWatk());
        }
        if (chr.isGM() && GameConfig.getServerBoolean("use_debug_damage_max")) chr.yellowMessage("[调试信息] calculateMaxBaseDamage_calcDmgMax: " + calcDmgMax);
        ...
        if (ret.skill != 0) {
            ...
            if (magic) {
                ...
                if (chr.getJob() == Job.IL_ARCHMAGE || chr.getJob() == Job.IL_MAGE) {//冰雷魔力激化
                ...
                } else if (chr.getJob() == Job.FP_ARCHMAGE || chr.getJob() == Job.FP_MAGE) {//火毒魔力激化
                ...
                } else if (chr.getJob() == Job.BLAZEWIZARD3 || chr.getJob() == Job.BLAZEWIZARD4) {//炎术士魔力激化
                ...
                } else if (chr.getJob() == Job.EVAN7 || chr.getJob() == Job.EVAN8 || chr.getJob() == Job.EVAN9 || chr.getJob() == Job.EVAN10) {//龙神？？？？
            ...
            } else if (ret.skill == Hermit.SHADOW_MESO) {//金钱攻击
        ...
        if (chr.isGM() && GameConfig.getServerBoolean("use_debug_damage_max")) chr.yellowMessage("[调试信息] skillEffect_calcDmgMax = " + calcDmgMax);
        ...
        if (comboBuff != null && comboBuff > 0) {
            ...
            int skillLvAdv = chr.getSkillLevel(advcomboid);

            if (skillLvAdv > 0) {// 进阶斗气
                StatEffect ceffect = SkillFactory.getSkill(advcomboid).getEffect(skillLvAdv);
                if (comboBuff > 6) calcDmgMax = (long) Math.floor(calcDmgMax * ((ceffect.getDamage() / 100.0) + (0.20 + ((comboBuff - 5) * 0.04))));
                else calcDmgMax = (long) Math.floor(calcDmgMax * ((ceffect.getDamage() / 100.0) + 0.20));
            } else {// 斗气集中
                StatEffect ceffect = SkillFactory.getSkill(oid).getEffect(chr.getSkillLevel(oid));
                calcDmgMax = (long) Math.floor(calcDmgMax * ((ceffect.getDamage() / 100.0) + ((comboBuff - 1) * 0.05)));
            }

            if (GameConstants.isFinisherSkill(ret.skill)) {// 狂乱剑/斧、气绝剑/斧
        ...
        if (chr.getEnergyBar() == 15000) {//能量获得效果
...
//            calcDmgMax *= (100 + ceffect.getDamage()) / 100;
        ...
        for (PlayerBuffValueHolder pbvh : chr.getAllBuffs()) {
            int bonusDmg;
            if (chr.getJob().getId() == 522) bonusDmg = pbvh.effect.getX();
            else bonusDmg = pbvh.effect.getDamage() - 100;
            bonusDmgBuff += bonusDmg;
        }

        if (bonusDmgBuff != 100 && comboBuff == null) {
        ...
        boolean canCrit = jobid == 300 || jobid == 310 || jobid == 311 || jobid == 312 || jobid == 320 || jobid == 321 || jobid == 322 || jobid == 410 || jobid == 411 || jobid == 412;

        StatEffect sharpEyesEffect = chr.getBuffEffect(BuffStat.SHARP_EYES);
        if (sharpEyesEffect != null) canCrit = true;

        if (canCrit) {
            if (sharpEyesEffect != null) {
                int index = sharpEyesEffect.getY();
                if (index < 100) index += 100; //适配神射手和箭神火眼在wz里数值逻辑不同
                int skilLv = chr.getSkillLevel(Assassin.CRITICAL_THROW);
                // 精确火眼按照当前等级计算伤害，而不是直接粗暴的取满级1.4，如果技改了wz，也能完全适配
                calcDmgMax = (long) (((index + (skilLv <= 30 ? 100 : skilLv == 31 ? 120 : skilLv == 32 ? 150 : 100)) / 100.00) * calcDmgMax);
                if (chr.isGM() && GameConfig.getServerBoolean("use_debug_damage_max")) chr.yellowMessage("[调试信息] sharpEyesEffect_calcDmgMax = " + calcDmgMax);
            } else {
                double rate = 2;
                int skilLv = chr.getSkillLevel(Assassin.CRITICAL_THROW);
                if (skilLv == 31) rate = 2.2;
                else if (skilLv == 32) rate = 2.5;
                calcDmgMax *= (long) rate;
            }
        }

        StatEffect shadowP = chr.getBuffEffect(BuffStat.SHADOWPARTNER);
        boolean shadowPartner = shadowP != null;
        if (ret.skill != 0) {
        ...
        for (int i = 0; i < ret.numAttacked; i++) {
            ...
            long currentDmgMax = calcDmgMax;

            if (chr.getBuffEffect(BuffStat.WK_CHARGE) != null) {//圣骑士属性克制计算
                // Charge, so now we need to check elemental effectiveness
                int sourceID = chr.getBuffSource(BuffStat.WK_CHARGE);
                int level = chr.getSkillLevel(sourceID);
                if (monster != null) {
                    if (sourceID == WhiteKnight.BW_FIRE_CHARGE || sourceID == WhiteKnight.SWORD_FIRE_CHARGE) {
                        if (monster.getStats().getEffectiveness(Element.FIRE) == ElementalEffectiveness.WEAK) {//火属性
                            currentDmgMax *= (1.05 + level * 0.015);
                        }
                    } else if (sourceID == WhiteKnight.BW_ICE_CHARGE || sourceID == WhiteKnight.SWORD_ICE_CHARGE) {//冰属性
                        if (monster.getStats().getEffectiveness(Element.ICE) == ElementalEffectiveness.WEAK) {
                            currentDmgMax *= (1.05 + level * 0.015);
                        }
                    } else if (sourceID == WhiteKnight.BW_LIT_CHARGE || sourceID == WhiteKnight.SWORD_LIT_CHARGE) {//雷电属性
                        if (monster.getStats().getEffectiveness(Element.LIGHTING) == ElementalEffectiveness.WEAK) {
                            currentDmgMax *= (1.05 + level * 0.015);
                        }
                    } else if (sourceID == Paladin.BW_HOLY_CHARGE || sourceID == Paladin.SWORD_HOLY_CHARGE) {//圣属性
                        if (monster.getStats().getEffectiveness(Element.HOLY) == ElementalEffectiveness.WEAK) {
                            currentDmgMax *= (1.2 + level * 0.015);
                        }
                    }
                } else {
            ...
            if (ret.skill != 0) {
                ...
                if (ret.skill == Hermit.SHADOW_WEB) {//影网术
                    if (monster != null) {
                        currentDmgMax = monster.getHp() / (50 - chr.getSkillLevel(skill));
                    }
                } else if (ret.skill == Hermit.SHADOW_MESO) {//金钱攻击
                    if (monster != null) {
                        monster.debuffMob(Hermit.SHADOW_MESO);
                    }
                } else if (ret.skill == Aran.BODY_PRESSURE) {//抗压
                    if (monster != null) {
                        ...
                        if (bodyPressureDmg > currentDmgMax) {
                            currentDmgMax = bodyPressureDmg;
                        }
                    }
                } else if (ret.skill == Hunter.ARROW_BOMB) {//爆炸箭
                    if (monster != null) {
                        currentDmgMax *= 1.6;
                    }
                }
//                else if (ret.skill == Shadower.ASSASSINATE) {//暗杀
//                    if (monster != null) {
//                        currentDmgMax *= 1.5;
//                    }
//                }
            ...
            if (chr.getJob().getId() == 132 && chr.getSkillLevel(DarkKnight.BERSERK) > 0){//恶龙附身
                currentDmgMax *= SkillFactory.getSkill(DarkKnight.BERSERK).getEffect(chr.getSkillLevel(DarkKnight.BERSERK)).getDamage();
            }

            for (int j = 0; j < ret.numDamage; j++) {
                int damage = p.readInt();
                long hitDmgMax = currentDmgMax;
                if (ret.skill == Buccaneer.BARRAGE || ret.skill == ThunderBreaker.BARRAGE) { //光速拳
                    if (j > 3) {
                ...
                if (shadowPartner) {//影分身
                    ...
                    if (j >= ret.numDamage / 2) {
                        int skillLv = chr.getSkillLevel(Hermit.SHADOW_PARTNER);
                        if (skillLv <= 30) hitDmgMax *= 0.5;
                        else if (skillLv == 31) hitDmgMax *= 0.75;
                        else if (skillLv == 32) hitDmgMax *= 1.0;
                    }
                }
...
//                }
                ...
                if (chr.isGM() && GameConfig.getServerBoolean("use_debug_damage_max")){
                    chr.yellowMessage("[调试信息] SkillId = [" + ret.skill + "], Dmg = [" + damage + "], currentDmgMax = [" + currentDmgMax + "], hitDmgMax = [" + hitDmgMax + "]");
                }
                // Warn if the damage is over 1.5x what we calculated above.
                if (damage > hitDmgMax) {
                    AutobanFactory.DAMAGE_HACK.alert(chr, "DMG: " + damage + " MaxDMG: " + hitDmgMax + " SkillID: " + ret.skill + " MobID: " + (monster != null ? monster.getId() : "null"));
                }

                // Add a ab point if its over 3x what we calculated.
                if (damage > hitDmgMax * 1.2) {
                    damage = (int) hitDmgMax;
                    AutobanFactory.DAMAGE_HACK.addPoint(chr.getAutoBanManager(), "DMG: " + damage + " MaxDMG: " + hitDmgMax + " SkillID: " + ret.skill + " MobID: " + (monster != null ? monster.getId() : "null"));
                }

                if (damage > hitDmgMax * 1.5) {
                    AutobanFactory.DAMAGE_HACK.autoban(chr, "DMG: " + damage + " MaxDMG: " + hitDmgMax + " SkillID: " + ret.skill + " MobID: " + (monster != null ? monster.getId() : "null") );
                }
                ...
                if(effect != null) {
                    ...
                    if (ret.numDamage > maxattack) {
                        AutobanFactory.DAMAGE_HACK.addPoint(chr.getAutoBanManager(), "攻击段数过高: " + ret.numDamage + " 最高段数: " + maxattack + " SkillID: " + ret.skill + " MobID: " + (monster != null ? monster.getId() : "null"));
                    }
                }
