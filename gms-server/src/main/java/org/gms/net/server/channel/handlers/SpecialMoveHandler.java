...
public final class SpecialMoveHandler extends AbstractPacketHandler {
    ...
    public final void handlePacket(InPacket p, Client c) {
        ...
        */
        ...
        if (Beginner.isSamsaraSkill(skillid)) {
            chr.summonSamsara((byte) 1);
        } else {
            StatEffect effect = skill.getEffect(skillLevel);
            if (effect.getCooldown() > 0) {
                if (chr.skillIsCooling(skillid)) {
                    return;
                } else if (skillid != Corsair.BATTLE_SHIP && inCoolDown) {
                    int cooldownTime = effect.getCooldown();
                    if (GameConfig.getServerBoolean("cooldown_zero_gm") && chr.isGM()) cooldownTime = 0;
