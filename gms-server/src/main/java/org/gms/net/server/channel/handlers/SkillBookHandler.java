            ...
            try {
                ...
                if (skilldata.get("skillid") == 0) {
                    ...
                    if (ItemInformationProvider.rollSuccessChance(skilldata.get("success"))) {
                        success = true;
                        if (skill2.getId() == 1013) player.changeSkillLevel(skill2, (byte) 1, 1, -1);
                        else player.changeSkillLevel(skill2, player.getSkillLevel(skill2), Math.max(skilldata.get("masterLevel"), player.getMasterLevel(skill2)), -1);
                ...
                if (GameConfig.getServerBoolean("broadcast_skill_book_result") && canuse){
                    String msg = "";
                    String skillName = SkillFactory.getSkill(skill2.getId()).getName();
                    if (success) msg = "[能手册] :  " + player.getName() + "  成功将  < " + skillName + " >  最高等级提高到 " + Math.max(skilldata.get("masterLevel"), player.getMasterLevel(skill2)) + " 级";
                    else msg = "[能手册] :  " + player.getName() + "  尝试升级  < " + skillName + " >  时失败了, 恭喜TA吧~";
                    Server.getInstance().broadcastMessage(c.getWorld(), PacketCreator.serverNotice(3, c.getChannel(), msg, false));
                }
