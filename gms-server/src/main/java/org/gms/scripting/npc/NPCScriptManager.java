...
public class NPCScriptManager extends AbstractScriptManager {
    ...
    private boolean start(Client c, int npc, int oid, String fileName, Character chr, boolean itemScript, String engineName) {
        try {
            ...
            if (c.canClickNPC()) {
                if (fileName != null && GameConfig.getServerBoolean("use_debug") && c.getPlayer().isGM()) {
                    String msg = "[脚本信息] 打开脚本：< " + fileName + ".js >";
                ...
                if (!itemScript) {
                    if (fileName != null) {
                        engine = getInvocableScriptEngine("npc/" + fileName + ".js", c);
                        if (engine == null) {
                            engine = getInvocableScriptEngine("special/" + fileName + ".js", c);
                        }
                    }
                } else {
