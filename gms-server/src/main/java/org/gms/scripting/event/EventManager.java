...
//        return (int) (BossTime * GameConfig.getServerFloat("boss_respawn_mob_time_rate"));
        // 1. 计算基础时间（原逻辑：基础时间 × 配置倍率）
        float rate = GameConfig.getServerFloat("boss_respawn_mob_time_rate");
        int baseTime = (int) (BossTime * rate);

        // 2. 生成±10分钟（±600秒）的随机偏移量
        // nextInt(601) 生成0-600的随机数，减300后范围为-300到300
        int randomOffset = ThreadLocalRandom.current().nextInt(601) - 300;

        // 3. 计算最终时间，确保不小于0（避免负时间）
        int finalTime = baseTime + randomOffset * 1000;
        return Math.max(finalTime, 0);
    }
    /**
     * 填充EIM队列
     */
