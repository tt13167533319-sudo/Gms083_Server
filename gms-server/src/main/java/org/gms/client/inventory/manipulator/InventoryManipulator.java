...
public class InventoryManipulator {
    ...
    public static void move(Client c, InventoryType type, short src, short dst) {
        ...
        if (GameConfig.getServerBoolean("use_debug") && c.getPlayer().isGM()) { // 假设isGM()是检查玩家是否是管理员的方法
            ...
            c.getPlayer().dropMessage(5, "[debug] " + I18nUtil.getMessage("InventoryManipulator.handlePacket.message1")  + itemID);
        }
    }
    ...
    public static void equip(Client c, short src, short dst) {
        ...
        if (GameConfig.getServerBoolean("use_debug") && c.getPlayer().isGM()) { // 假设isGM()是检查玩家是否是管理员的方法
            int itemID = source.getItemId();
            c.getPlayer().dropMessage(5, "[debug] 道具ID：" + itemID + "，src：{ " + eqpInv.getType().getName() + "(" + eqpInv.getType().getType() + ")：" + src + " }，dst：{" + eqpdInv.getType().getName() + "(" + eqpdInv.getType().getType() + ")：" + dst + "}");
        }
    }
