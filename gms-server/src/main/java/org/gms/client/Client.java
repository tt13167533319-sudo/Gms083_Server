...
public class Client extends ChannelInboundHandlerAdapter {
    ...
    private void closeMapleSession() {
        ...
        try {
            ...
            if (!inTransition) {
                SessionCoordinator.getInstance().removeHostHwidEntryAndGetHwid(this.getRemoteAddress());
    ...
    private void removePlayer(World wserv, boolean serverTransition) {
        try {
            ...
            log.error("账号ID [{}] 卡号", player.getAccountId(), t);
        }
    }
    ...
    private void disconnectInternal(boolean shutdown, boolean cashshop) {//once per Client instance
        if (player != null && player.isLoggedIn() && player.getClient() != null) {
            player.stopAttack("角色下线");
            player.clearSamsaraMob();
            player.stopAutoUseSkillBuff();
            ...
            try {
                ...
                log.error("账号ID [{}] 卡号", player.getAccountId(), e);
            } finally {
...
//                        disconnectSession();
    ...
    public void changeChannel(int channel) {
        ...
        player.setInMapTimeRecord(System.currentTimeMillis());
        try {
