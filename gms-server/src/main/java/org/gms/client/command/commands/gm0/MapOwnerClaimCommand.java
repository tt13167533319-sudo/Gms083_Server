        ...
        if (c.tryacquireClient()) {
            try {
                ...
                if (GameConfig.getServerBoolean("use_map_ownership_system")) {
                    if (chr.getMap().getMapPlayers().size() <= 1) {
                        if (chr.getEventInstance() == null) {
                            MapleMap map = chr.getMap();
                            if (map.countBosses() >= 0 || MapId.isBossMap(map.getId())) {   // thanks Conrad for suggesting bosses prevent map leasing
                                ...
                                chr.getAbstractPlayerInteraction().openNpc(9010000,"获取地图归属");
//                                if (map.claimOwnership(chr)) {
//                                    //chr.dropMessage(5, I18nUtil.getMessage("MapOwnerClaimCommand.message3"));
//                                    chr.dropMessage(5, "您已获得地图归属权，您的队伍将有权独自在此狩猎，除非离开地图或挂机" + TimeUtil.formatTime(GameConfig.getServerShort("use_map_ownership_system_duration") * 60) + "以上");
//                                } else {
...
//                                }
                        ...
                        chr.dropMessage(5, I18nUtil.getMessage("MapOwnerClaimCommand.message8"));
                    }
                } else {
