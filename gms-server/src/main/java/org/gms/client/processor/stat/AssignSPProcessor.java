...
public class AssignSPProcessor {
    ...
    public static void SPAssignAction(Client c, int skillid) {
        ...
        try {
            ...
            } else if (Beginner.isSamsaraSkill(skillid)) {
                player.yellowMessage("[轮回碑石] 请先完成第一次转职后再试");
                NPCScriptManager.getInstance().dispose(c);
                QuestScriptManager.getInstance().dispose(c);
                c.sendPacket(PacketCreator.enableActions());
                c.removeClickedNPC();
            } else {
                if (skill.isBreakSkill()) player.dropMessage(5,"[技能突破] 请通过对应的 \"技能突破书\" 获取更高等级的技能");
                else player.dropMessage(5,"[错误提示] 技能加点出现未知错误，请联系管理员");
                NPCScriptManager.getInstance().dispose(c);
                QuestScriptManager.getInstance().dispose(c);
                c.sendPacket(PacketCreator.enableActions());
                c.removeClickedNPC();
