...
//            p.writeFixedString(StringUtil.getRightPaddedStr(giftMessage, '\0', 73));
    ...
    /*
    00	Due to an unknown error, failed	未知错误导致操作失败，请重试
    A3	Request timed out. Please try again.	请求超时，请重试
    A4	Due to an unknown error, failed + warpout	未知错误导致操作失败，已自动传送至安全区域
    A5	You don't have enough cash.	您的 NX 点数不足
    A6	long as shet msg	提示信息内容过长
    A7	You have exceeded the allotted limit of price for gifts.	您已超出赠送道具的价格限制
    A8	You cannot send a gift to your own account. Log in on the char and purchase	无法向自己的账号赠送道具，请登录对应角色直接购买
    A9	Please confirm whether the character's name is correct.	请确认角色名称是否正确
    AA	Gender restriction!	性别限制！（该道具仅限特定性别角色使用）
    AB	gift cannot be sent because recipient inv is full	接收方背包已满，无法赠送道具
    AC	exceeded the number of cash items you can have	您的现金道具持有数量已达上限
    AD	check and see if the character name is wrong or there is gender restrictions	请检查角色名称是否错误或存在性别限制
    B0	Wrong Coupon Code	兑换码错误
    B1	Disconnect from CS because of 3 wrong coupon codes < lol	兑换码连续错误 3 次，已退出现金商店
    B2	Expired Coupon	兑换码已过期
    B3	Coupon has been used already	该兑换码已被使用
    B4	Nexon internet cafes? lolfk	仅限 Nexon 合作网吧使用（私服可译为 “仅限指定网吧 IP 使用”）
    B8	Due to gender restrictions, the coupon cannot be used.	性别限制，无法使用该兑换码
    BB	inv full	背包已满
    BC	long as shet "(not?) available to purchase by a use at the premium" msg	高级会员专属道具，仅限高级会员购买（提示信息内容过长）
    BD	invalid gift recipient	无效的赠送对象
    BE	invalid receiver name	接收方角色名称无效
    BF	item unavailable to purchase at this hour	该道具当前时段暂不支持购买
    C0	not enough items in stock, therefore not available	道具库存不足，暂无法购买
    C1	you have exceeded spending limit of NX	您已超出 NX 点数消费限额
    C2	not enough mesos? Lol not even 1 mesos xD	金币不足（甚至不足 1 枚金币）
    C3	cash shop unavailable during beta phase	测试阶段暂不开放现金商店
    C4	check birthday code	请检查生日验证码是否正确
    C7	only available to users buying cash item, whatever msg too long	仅限现金道具购买用户使用（提示信息内容过长）
    C8	already applied for this	您已申请过该道具 / 活动
    CD	You have reached the daily purchase limit for the cash shop.	您已达现金商店每日购买限额
    D0	coupon account limit reached	该兑换码已达账号使用上限
    D2	coupon system currently unavailable	兑换码系统当前暂不可用
    D3	item can only be used 15 days after registration	账号注册满 15 天后才可使用该道具
    D4	not enough gift tokens	赠送代币不足
    D6	fresh people cannot gift items lul	新注册用户暂无法赠送道具
    D7	bad people cannot gift items >:(	违规用户无法赠送道具 >:(
    D8	cannot gift due to limitations	因限制规则无法赠送道具
    D9	cannot gift due to amount of gifted times	已达每日赠送次数上限
    DA	cannot be gifted due to technical difficulties	因技术问题暂无法接收赠送道具
    DB	cannot transfer to char below level 20	无法转移道具至 20 级以下角色
    DC	cannot transfer char to same world	无法在同一服务器内转移角色
    DD	cannot transfer char to new server world	无法将角色转移至新服务器
    DE	cannot transfer char out of this world	该服务器角色暂不支持转出
    DF	cannot transfer char due to no empty char slots	目标账号角色栏无空位，无法转移角色
    E0	event or free test time ended	活动或免费测试时间已结束
    E6	item cannot be purchased with MaplePoints	该道具无法使用枫叶点数购买
    E7	lol sorry for the inconvenience, eh?	给您带来不便，敬请谅解
    E8	cannot purchase by anyone under 7	7 岁以下用户无法购买该道具
    */
